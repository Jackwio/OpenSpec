...
此規範定義 OpenSpec 如何解析、讀取和寫入使用者級全域設定。它管轄 `src/core/global-config.ts` 模組，它為儲存跨專案持續存在的使用者首選項、功能標誌和設定提供了基礎。該規範透過遵循 XDG 基本目錄規範和特定於平台的回退來確保跨平台相容性，並透過架構演化規則保證向前/向後相容性。
## 要求
### 要求：全域設定存儲
